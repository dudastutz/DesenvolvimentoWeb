...
<head>
    ...
    <script src="scriptaula04.js"></script>
...
<body>
    ...
    // Filtrar resultados
    $resultado = array_filter($pessoas, function($pessoa) use ($nome, $idade, $cidade, $estado) {
        if (!empty($nome) && stripos($pessoa['nome'], $nome) === false) {
            return false;
        }
        if (!empty($cidade) && $pessoa['cidade'] !== $cidade) {
            return false;
        }
        if (!empty($estado) && $pessoa['estado'] !== $estado) {
            return false;
        }
        if (!empty($idade)) {
    ...
    if (!empty($resultado)) {
        ...
        echo "<tr><th>Nome</th><th>Idade</th><th>Cidade</th><th>Estado</th></tr>";
        foreach ($resultado as $pessoa) {
            $idadePessoa = calculaIdade($pessoa['dtNascimento']);
            echo "<tr>";
            echo "<td>{$pessoa['nome']}</td>";
            echo "<td>{$idadePessoa}</td>";
